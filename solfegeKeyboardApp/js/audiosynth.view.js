/*Link doSelect and doList; add labels:
    var i = doSelect
	if i - adjustor < 0:
        adjustor += 12
    if i + adjustor >= 12:
        adjustor -= 12
	doList[notes.index(n)]
*/
function AudioSynthView() {
	...
	var reverseLookup = {};
	var solfegeList = ["Do", "Di/Ra", "Re", "Ri/Me", "Mi", "Fa", "Fi/Se", "So", "Si/Le", "La", "Li/Te", "Ti", "Do", "Di/Ra", "Re", "Ri/Me", "Mi", "Fa", "Fi/Se", "So", "Si/Le", "La", "Li/Te", "Ti", "Do", "Di/Ra", "Re", "Ri/Me", "Mi", "Fa", "Fi/Se", "So", "Si/Le", "La", "Li/Te", "Ti"];
	...
	var fnCreateKeyboard = function(keyboardElement) {
		...
		selectDo = document.getElementById('do-selector');
		...
		var adjustor = selectDo

		for(var i=-1;i<=1;i++) {
			for(var n in notes) {
				if(n[2]!='b') {
					var thisKey = document.createElement('div');
					if(n.length>1) { //note names with sharps or flats are more than one character
						thisKey.className = 'black key';
						thisKey.style.width = '33px';
						thisKey.style.height = '132px';
						thisKey.style.left = (44 * (iWhite - 1)) + 27.5 - 42 + 'px'; //left boundary of key
					} else {
						thisKey.className = 'white key';
						thisKey.style.width = '44px';
						thisKey.style.height = '220px';
						thisKey.style.left = 44 * iWhite - 42 + 'px';
						iWhite++;
					}
					var label = document.createElement('div');
					label.className = 'label';
					label.innerHTML = '<b>' + String.fromCharCode(reverseLookupText[n + ',' + i]) + '</b>' + '<br /><br />' + n.substr(0,1) +
					'<span name="OCTAVE_LABEL" value="' + i + '"></span>' + (n.substr(1,1)?n.substr(1,5):'') + '<br><br>' + solfegeList[notesArray.indexOf(n) - selectDo.value + 12];
